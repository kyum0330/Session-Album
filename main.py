...
1) [내부 구상]: 이 키워드들을 엮어서 만들 수 있는 매력적인 스토리라인과 시각적 테마를 스스로 3가지 정도 깊이 있게 브레인스토밍 해보고, 곡의 기승전결에 따른 구간별 감정 흐름(Emotional Flow)까지 치밀하게 설계해 봐. (이 과정은 너의 내부 추론을 위한 것이며 출력하지 않아도 됨)
2) [최종 도출]: 네가 구상한 아이디어 중 가장 훌륭하고 트렌디한 1가지를 확정해.
3) [포맷 출력]: 확정한 아이디어를 바탕으로, 시스템 프롬프트에서 요구한 ###DETAIL### 부터 ###UPLOAD### 까지의 필수 구분자 포맷에 맞추어 완벽한 최종 결과물만 출력해. 이때 다음 두 가지를 반드시 반영해 줘:
   - ###SUNO### (Style of Music): 곡의 장르와 함께 곡을 관통하는 핵심 분위기와 감정을 묘사하는 간결한 영어 키워드를 포함할 것.
   - ###LYRICS###: 네가 설계한 구간별 감정선이 Suno AI에 직접 전달되도록, 각 단락(Verse, Chorus 등) 시작 부분의 대괄호 [ ] 메타 태그 안에 곡의 구조뿐만 아니라 보컬의 감정, 창법, 분위기를 구체적으로 명시할 것. (예: [Verse 1: Calm and melancholic vocal], [Chorus: Burst of hopeful emotion, powerful delivery])
</Action_Steps>
"""
